...
//            )
            ...
            is WithLocationPermissionState -> currentState = currentState.getNextState(
...
//            is WithLocationPermissionState -> enableLocationIfNeed()
    ...
    private fun initMainScreen() {
        ...
        viewHolder = WeatherViewHolder(mainContent).setup {
            disableRefreshing()
    ...
    private fun MainViewModel.observe() {
        locationLiveData.observe(this@WeatherActivity, Observer { response ->
            when (response) {
        ...
        weatherAndWhatToWearLiveData.observe(this@WeatherActivity, Observer { response ->
            when (response) {
                is Loading -> viewHolder.showRefreshing()
                is Success -> {
        ...
        if (PermissionUtils.isLocationPermissionGranted(this)) {
            ...
            supportFragmentManager.findFragmentByTag(RequestLocationDialog.TAG) as? RequestLocationDialog
                ?: RequestLocationDialog()
                    .also {
                        it.onPermitClick = {
                            PermissionUtils.requestLocationPermissionInActivity(
                                this,
